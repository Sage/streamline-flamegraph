...
exports.create = function(options) {
	...
	glob.emitter.on('yield', function() {
		...
		var f = glob.frame;
		if (!f) return;
		//console.log("YIELD " + f.name + ": id=" + f.id + ' now=' + time());		
	});
	glob.emitter.on('resume', function() {
		...
		if (!f) return;
		cpu = time();
		//console.log("RESUME " + f.name + ": id=" + f.id + ' ' + (cpu - f.exited) + ' now=' + time());		
		if (f.exited < cpu) {
	...
	glob.emitter.on('enter', function() {
		...
		//console.log("ENTER " + f.name + ": id=" + f.id + ' ' + (now - cpu) + ' now=' + now);
		if (cpu < now) {
	...
	glob.emitter.on('exit', function() {
		...
		if (!f) return;
		if (--f.recurse) return;
		//console.log("EXIT " + f.name + ": id=" + f.id + ' ' + (now - cpu) + ' now=' + now);
		if (cpu < now) {
			writer.write(perfStack(f, now - cpu));
			cpu = now;
		}
